...
<template>
  <div class="user-box">
    <s-header :name="'推荐课程'"></s-header>
    <div v-if="current == 0" class="box">
      推荐课程
    </div>
    <div v-if="current == 1" class="box">
      我的课程
    </div>
    ...
    <div>
      <header class="category-header wrap van-hairline--bottom">
        ...
        <div class="header-search">
          ...
          <router-link tag="span" class="search-title" to="./product-list?from=category">搜索课程</router-link>
        </div>
...
import { getCategory } from "@/service/course";
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      Toast.clear()
      state.categoryData = data.content
    })
